"""可解释的企业级 SSD 压力测试结果分析引擎。

该模块只使用任务已经采集的遥测样本和事件，不伪造 fio 性能数据。
评分用于形成可追溯的测试结论：温度、尾延迟、吞吐稳定性、健康度、
异常事件以及任务完成度均有独立的扣分和判定依据。
"""
...
def _number(value: Any) -> float | None:
    """Convert numeric telemetry values to floats; '--' and invalid values are ignored."""
...
def _series(samples: Iterable[dict[str, Any]], field: str) -> list[float]:
    return [value for sample in samples if (value := _number(sample.get(field))) is not None]
...
def _trend(values: list[float]) -> dict[str, float | None]:
    """Return a least-squares slope per sample and first-to-last drift percentage."""
    if len(values) < 2:
        return {"slope": None, "drift_pct": None}
    ...
    return {"slope": slope, "drift_pct": drift}


def _summary(values: list[float], unit: str, include_trend: bool = True) -> dict[str, Any]:
    if not values:
    ...
    data = {
        ...
        "p95": round(_percentile(values, 95) or 0, 2),
        "coefficient_variation": round(standard_deviation / average * 100, 2) if average else None,
    }
    if include_trend:
        trend = _trend(values)
        data["slope"] = round(trend["slope"], 4) if trend["slope"] is not None else None
        data["drift_pct"] = round(trend["drift_pct"], 2) if trend["drift_pct"] is not None else None
    return data


def _paired_series(samples: Iterable[dict[str, Any]], first: str, second: str) -> tuple[list[float], list[float]]:
    """Keep only sample pairs where both telemetry values are valid."""
    pairs = [(_number(sample.get(first)), _number(sample.get(second))) for sample in samples]
...
def _pearson(first: list[float], second: list[float]) -> float | None:
    ...
    first_size = sqrt(sum((left - first_mean) ** 2 for left in first))
    second_size = sqrt(sum((right - second_mean) ** 2 for right in second))
    if not first_size or not second_size:
        return None
    return numerator / (first_size * second_size)


def _ewma_anomalies(values: list[float], absolute_floor: float, span: int = 6) -> dict[str, Any]:
    """EWMA + 3σ detector for abrupt deviations in a telemetry sequence.

    The absolute floor prevents stable, low-noise sequences from producing
    meaningless alerts on a very small decimal change.
    """
    if len(values) < 6:
    ...
    assessment = "未发现突发异常" if not indices else f"发现 {len(indices)} 个 EWMA 异常点"
    return {
        "available": True,
        "count": len(indices),
        "indices": indices,
...
def _linear_projection(values: list[float], progress: float) -> dict[str, Any]:
    """Use least-squares regression to project the end-of-test metric trend."""
    ...
    return {
        ...
        "projected_change_pct": round(projected_change, 2) if projected_change is not None else None,
        "r_squared": round(r_squared, 3) if r_squared is not None else None,
        "confidence": confidence,
        "assessment": "基于当前采样的线性趋势外推；非设备寿命预测。",
    }


def _thermal_stability(samples: list[dict[str, Any]], threshold: float) -> dict[str, Any]:
    """Assess thermal exposure and its relation to throughput, if data permits."""
    temperatures, throughput = _paired_series(samples, "temperature", "throughput")
    if not temperatures:
        return {"available": False, "assessment": "缺少成对温度与吞吐数据，无法分析热稳定性"}
    near_limit = sum(value >= threshold - 5 for value in temperatures)
    correlation = _pearson(temperatures, throughput)
...
def _historical_baseline(task: dict[str, Any], history: Iterable[dict[str, Any]] | None) -> dict[str, Any]:
    """Compare against completed tasks with the same device and pressure profile."""
    if not history:
    ...
    candidates = [
        ...
        and all(item.get(field) == task.get(field) for field in match_fields)
    ]
    if len(candidates) < 3:
        return {"available": False, "count": len(candidates), "assessment": f"同设备同配置历史任务仅 {len(candidates)} 条，至少需要 3 条"}
    throughput_baseline = [mean(values) for item in candidates if (values := _series(item.get("samples") or [], "throughput"))]
    latency_baseline = [mean(values) for item in candidates if (values := _series(item.get("samples") or [], "p99"))]
...
def _deduct(score: int, points: int, reasons: list[str], message: str) -> int:
    if points > 0:
        reasons.append(f"-{points} 分：{message}")
    return max(0, score - points)


def _temperature_assessment(values: list[float], threshold: float) -> tuple[int, dict[str, Any], list[str]]:
    ...
    if maximum >= threshold:
        ...
        reasons.append(f"最高温度 {maximum:.1f}°C 达到或超过阈值 {threshold:.1f}°C")
    elif headroom < 3:
        penalty = 15
        metric["assessment"] = "温度接近阈值"
        reasons.append(f"最高温度 {maximum:.1f}°C，距阈值仅 {headroom:.1f}°C")
    elif headroom < 8:
        penalty = 7
        metric["assessment"] = "温度存在余量但需关注"
        reasons.append(f"最高温度 {maximum:.1f}°C，温度余量 {headroom:.1f}°C")
    else:
        metric["assessment"] = "温度余量充足"
        reasons.append(f"最高温度 {maximum:.1f}°C，低于阈值 {headroom:.1f}°C")
    ...
    if average >= threshold - 5 and maximum < threshold:
        penalty += 3
        reasons.append(f"平均温度 {average:.1f}°C 接近阈值")
    return min(penalty, 35), metric, reasons


def _latency_assessment(values: list[float]) -> tuple[int, dict[str, Any], list[str]]:
    ...
    reasons.append(f"P99 延迟均值 {average:.2f} ms，P95 {p95:.2f} ms，最高 {maximum:.2f} ms")
    if spikes:
        spike_penalty = min(8, spikes * 2)
        penalty += spike_penalty
        reasons.append(f"检测到 {spikes} 个明显 P99 延迟尖峰")
...
def _throughput_assessment(values: list[float]) -> tuple[int, dict[str, Any], list[str]]:
    ...
    drift_text = "数据不足" if drift is None else f"{drift:+.1f}%"
    reasons.append(f"平均吞吐 {metric['avg']:.1f} MB/s，首尾分段变化 {drift_text}")
    return penalty, metric, reasons


def _health_assessment(values: list[float]) -> tuple[int, dict[str, Any], list[str]]:
    ...
    reasons.append(f"最低介质健康度 {lowest:.1f}%")
    return penalty, metric, reasons


def _event_assessment(events: list[dict[str, Any]]) -> tuple[int, dict[str, Any], list[str]]:
    ...
    reasons = []
    if counts.get("严重"):
        reasons.append(f"记录到 {counts['严重']} 条严重事件")
    if counts.get("警告"):
        reasons.append(f"记录到 {counts['警告']} 条警告事件")
    if not reasons:
        reasons.append("未记录警告或严重事件")
    return penalty, {"counts": counts, "assessment": "存在异常事件" if penalty else "未发现告警事件"}, reasons


def _completion_assessment(task: dict[str, Any]) -> tuple[int, dict[str, Any], list[str]]:
    status = task.get("status", "未知")
    progress = _number(task.get("progress")) or 0
    if status == "已完成" and progress >= 100:
        return 0, {"status": status, "progress": progress, "assessment": "已完成"}, ["任务已完成全部配置时长"]
    if status in {"已停止", "已中断", "失败"}:
        return 45, {"status": status, "progress": progress, "assessment": "未完整执行"}, [f"任务状态为“{status}”，仅完成 {progress:.1f}%"]
    if status in {"运行中", "停止中", "排队中"}:
        return 25, {"status": status, "progress": progress, "assessment": "结果尚未定稿"}, [f"任务仍处于“{status}”状态，当前进度 {progress:.1f}%"]
    return 35, {"status": status, "progress": progress, "assessment": "状态未知"}, [f"任务状态“{status}”无法作为完成依据"]


def _risk_level(score: int, has_critical_event: bool, complete: bool) -> tuple[str, str]:
    if not complete:
        return "未定", "任务未完整结束，当前分析仅反映已采集阶段。"
    if has_critical_event or score < 65:
        return "高风险", "存在明显稳定性或执行风险，建议复测并排查设备与环境。"
    if score < 85:
        return "中风险", "测试已完成，但存在需要关注的风险指标。"
    return "低风险", "已采集指标未发现需要阻断验收的明显风险。"


def analyze_task(task: dict[str, Any], history: Iterable[dict[str, Any]] | None = None) -> dict[str, Any]:
    """Analyse one persisted task and return a JSON-serialisable evidence report.

    Score starts at 100. Each metric contributes a bounded penalty. Missing
    telemetry is recorded as an evidence limitation rather than treated as a
    fabricated good score.
    """
    ...
    completion_penalty, completion, completion_reasons = _completion_assessment(task)
    ...
    anomalies = {
        "temperature": _ewma_anomalies(temperature_values, absolute_floor=3),
        "latency": _ewma_anomalies(latency_values, absolute_floor=2),
        "throughput": _ewma_anomalies(throughput_values, absolute_floor=150),
    }
    projections = {
        "temperature": _linear_projection(temperature_values, progress),
        "latency": _linear_projection(latency_values, progress),
        "throughput": _linear_projection(throughput_values, progress),
    }
    ...
    score = _deduct(score, anomaly_penalty, deductions, f"EWMA 检测到 {anomaly_count} 个突发异常点")
    ...
    risk_level, risk_text = _risk_level(score, has_critical, complete)
    ...
    evidence = {
        "completion": completion_reasons,
        "temperature": temperature_reasons,
        "latency": latency_reasons,
        "throughput": throughput_reasons,
        "health": health_reasons,
        "events": event_reasons,
        "advanced": [
            f"EWMA 异常检测：温度 {anomalies['temperature']['count']} 个、延迟 {anomalies['latency']['count']} 个、吞吐 {anomalies['throughput']['count']} 个异常点。",
            thermal["assessment"],
            historical["assessment"],
        ],
    }
    return {
        "algorithm": "SSD Stability Score v2.0",
        "score": score,
        "risk_level": risk_level,
        "risk_text": risk_text,
        "conclusion": conclusion,
        "mode_notice": "安全演示结果仅用于系统流程验证" if is_demo else "真实 fio 结果可用于本次配置下的压力测试评价",
        "deductions": deductions or ["未触发评分扣分项"],
        "evidence": evidence,
