...
class AnalysisEngineTests(unittest.TestCase):
    ...
    def test_overtemperature_and_critical_event_fail(self):
        task = task_template(samples=[{'temperature': 71, 'p99': 60, 'throughput': 2500, 'health': 78}, {'temperature': 74, 'p99': 75, 'throughput': 1600, 'health': 78}, {'temperature': 76, 'p99': 90, 'throughput': 1400, 'health': 77}], events=[{'time': '2026-07-28 12:10:00', 'severity': '严重', 'text': '温度超限'}])
    ...
    def test_advanced_algorithms_detect_anomaly_and_compare_baseline(self):
        samples = [{'temperature': 40 + index, 'p99': 4 + index * 0.1, 'throughput': 2500 - index * 5, 'health': 98} for index in range(7)]
        samples.append({'temperature': 59, 'p99': 31, 'throughput': 1700, 'health': 98})
        task = task_template(samples=samples, progress=100)
        history = [task_template(id='history-{0}'.format(index), samples=[{'temperature': 42, 'p99': 4.2, 'throughput': 2500 + index * 10, 'health': 98} for _ in range(8)]) for index in range(3)]
    ...
    def test_report_renders_evidence_completeness_section(self):
        task = task_template(samples=[
            {'time': '2026-07-28 12:00:00', 'stage_name': '负载', 'temperature': 42, 'p99': 4.0, 'throughput': 2500, 'health': 98},
