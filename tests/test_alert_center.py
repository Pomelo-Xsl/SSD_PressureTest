...
from alerts import acknowledge_alert, alert_overview, alerts_for_tasks
...
class AlertCenterTests(unittest.TestCase):
    ...
    def test_alerts_include_only_warning_and_critical_events(self):
        alerts = alerts_for_tasks([self.task])
        self.assertEqual(len(alerts), 2)
        self.assertEqual(alerts[0]['severity'], '严重')
        summary = alert_overview(alerts)
    ...
    def test_acknowledge_marks_task_alert(self):
        ...
        self.assertTrue(alerts_for_tasks([self.task])[1]['acknowledged'])
    ...
    def test_event_identifier_survives_event_reordering(self):
        ...
        current = next(alert for alert in alerts_for_tasks([self.task]) if alert['id'] == 'task-a:event-latency')
